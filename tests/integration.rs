...
fn curl(args: &[&str]) -> (u16, String, String) {
    ...
    cmd.arg("-sk") // silent + insecure (self-signed cert)
        .arg("--max-time").arg("10")
        .arg("-D").arg("/dev/stderr") // headers to stderr
        .args(args);
...
fn get(path: &str) -> (u16, String, String) {
    curl(&["-H", "Host: bench.local", &format!("https://127.0.0.1:4433{}", path)])
}
...
integration_test!(t01_get_root_returns_200, {
    ...
    assert!(body.contains("<h1>Zion Test Backend</h1>"), "should contain test backend HTML");
});
...
integration_test!(t02_api_get_returns_json, {
    ...
    assert!(body.contains("\"status\":\"ok\""), "API should return JSON with status:ok");
});
...
integration_test!(t04_forwarding_headers, {
    ...
    assert!(body.contains("\"x_forwarded_for\":\"127.0.0.1\""), "X-Forwarded-For missing");
    assert!(body.contains("\"x_real_ip\":\"127.0.0.1\""), "X-Real-IP missing");
    assert!(body.contains("\"x_forwarded_proto\":\"https\""), "X-Forwarded-Proto missing");
});
...
integration_test!(t09_static_cache_serves_asset, {
    ...
    assert!(headers.to_lowercase().contains("immutable"), "should have Cache-Control immutable");
});
...
integration_test!(t13_large_response_512kb, {
    ...
    assert_eq!(status, 200);
    assert!(body.len() >= 500_000, "large response should be >= 500KB, got {} bytes", body.len());
});
...
integration_test!(t16_security_headers_present, {
    ...
    assert!(h.contains("x-content-type-options"), "X-Content-Type-Options missing");
...
integration_test!(t17_healthz_returns_200, {
    let (status, body, _) = curl(&[
        "-H", "Host: bench.local",
        "https://127.0.0.1:4433/healthz",
    ]);
    assert_eq!(status, 200);
...
integration_test!(t19_http_redirects_to_https, {
    ...
    assert!(headers.to_lowercase().contains("location: https://"), "should redirect to https://");
});
