...
fn prepare_request<B>(
    ...
    *req.version_mut() = Version::HTTP_11;
    ...
    req.headers_mut().insert(
        "X-Forwarded-Proto",
        if proto == "https" { PROTO_HTTPS.clone() } else { PROTO_HTTP.clone() },
    );
...
async fn send_request(
    ...
    match client.request(req).await {
        Ok(resp) => {
            crate::metrics::METRICS.upstream_duration.observe(upstream_start.elapsed());
...
async fn send_ws_upgrade(
    sender: &mut hyper::client::conn::http1::SendRequest<Incoming>,
    req: Request<Incoming>,
    on_client_upgrade: hyper::upgrade::OnUpgrade,
) -> Result<Response<ZionBody>, hyper::Error> {
    ...
    // Spawn bidirectional pipe with idle timeout — runs after client receives our 101.
    // 30-minute timeout prevents resource exhaustion from abandoned WebSocket connections.
    tokio::spawn(async move {
        if let Ok(client_upgraded) = on_client_upgrade.await {
            ...
            let _ = tokio::time::timeout(
                std::time::Duration::from_secs(30 * 60), // 30 min max session
...
mod tests {
    ...
    fn scheme() -> Scheme { "http".parse().unwrap() }
    fn authority() -> Authority { "127.0.0.1:8000".parse().unwrap() }
    fn authority2() -> Authority { "127.0.0.1:3000".parse().unwrap() }
    ...
    fn prepare_rewrites_uri() {
        ...
        assert_eq!(req.uri().to_string(), "http://127.0.0.1:8000/api/v1/users?page=2");
    }
    ...
    fn prepare_preserves_query_string() {
        ...
        assert_eq!(req.uri().to_string(), "http://127.0.0.1:3000/search?q=hello&page=1");
    }

    #[test]
    fn prepare_root_path_default() {
        let req = Request::builder().method("GET").uri("http://example.com").body(()).unwrap();
    ...
    fn default_port(scheme: &str) -> u16 {
        if is_tls_upstream(scheme) { 443 } else { 80 }
    }
