...
pub fn spawn_renewal_task(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let needs_renewal = check_cert_expiry(
                &tls_config.cert_path,
                acme_config.renew_before_days,
            );

            if needs_renewal {
                crate::logging::info("acme", &format!(
                ...
                match do_renewal(&acme_config, &challenge_store, &tls_config).await {
                    Ok(()) => {
                        ...
                        match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                            crate::tls::load_tls_config(&tls_config)
                        })) {
                            Ok(new_config) => {
                                let new_acceptor = tokio_rustls::TlsAcceptor::from(
                                    Arc::new(new_config)
                                );
                                tls_acceptor.store(Arc::new(new_acceptor));
                                crate::logging::info("acme", "TLS hot-reloaded with new certificate");
                            }
                            Err(_) => {
                                crate::logging::error("acme", "failed to load renewed certificate");
                            }
                        }
                    }
                    Err(e) => {
...
async fn do_renewal_native(
    ...
    use instant_acme::{
        Account, AuthorizationStatus, ChallengeType,
    ...
    crate::logging::info("acme", &format!(
        "ACME order created (status: {:?})", order.state().status
    ));
    ...
    while let Some(result) = authorizations.next().await {
        ...
        crate::logging::info("acme", &format!(
            "serving HTTP-01 challenge for token: {}...", &token[..8.min(token.len())]
        ));

        // Insert into shared store — the HTTP handler serves this at
    ...
    let private_key_pem = order.finalize()
        .await
...
async fn do_renewal_script(config: &crate::config::AcmeConfig) -> Result<(), String> {
    ...
    let metadata = std::fs::metadata(&script)
        .map_err(|e| format!("cannot stat renew.sh: {}", e))?;
