...
fn default_acme_state_dir() -> String { "/etc/zion/acme".to_string() }

fn default_true() -> bool { true }
fn default_tls_min_version() -> String { "1.3".to_string() }
fn default_alpn() -> Vec<String> { vec!["h2".to_string(), "http/1.1".to_string()] }
...
fn default_connect_timeout() -> u64 { 3000 }
fn default_keepalive() -> usize { 64 }
pub(crate) fn default_client_auth() -> String { "none".to_string() }
...
fn default_max_body_mb() -> u64 { 10 }
fn default_max_depth() -> usize { 10 }
fn default_max_string_len() -> usize { 1_048_576 }
...
fn default_cache_mode() -> CacheMode { CacheMode::Memory }
fn default_max_entries() -> usize { 10_000 }
fn default_ttl() -> u64 { 31_536_000 } // 1 year
...
pub fn load_config(path: &str) -> ZionConfig {
    let raw = fs::read_to_string(path).unwrap_or_else(|e| panic!("Cannot read {}: {}", path, e));
    let config: ZionConfig = toml::from_str(&raw).unwrap_or_else(|e| panic!("Invalid TOML in {}: {}", path, e));
    validate_config(&config, path);
    config
}

/// Validate config at startup — fail fast with actionable error messages.
fn validate_config(config: &ZionConfig, path: &str) {
    let mut errors: Vec<String> = Vec::new();

    // Server addresses must parse
    if config.server.listen_http.parse::<std::net::SocketAddr>().is_err() {
        errors.push(format!("server.listen_http '{}' is not a valid address", config.server.listen_http));
    }
    if config.server.listen_https.parse::<std::net::SocketAddr>().is_err() {
        errors.push(format!("server.listen_https '{}' is not a valid address", config.server.listen_https));
    }

    // TLS cert files must exist
    if !std::path::Path::new(&config.tls.cert_path).exists() {
        errors.push(format!("tls.cert_path '{}' does not exist", config.tls.cert_path));
    }
    if !std::path::Path::new(&config.tls.key_path).exists() {
        errors.push(format!("tls.key_path '{}' does not exist", config.tls.key_path));
    }

    // SNI cert files must exist
    for (i, sni) in config.tls.sni.iter().enumerate() {
        if !std::path::Path::new(&sni.cert_path).exists() {
            errors.push(format!("tls.sni[{}] cert_path '{}' does not exist", i, sni.cert_path));
        }
        if !std::path::Path::new(&sni.key_path).exists() {
            errors.push(format!("tls.sni[{}] key_path '{}' does not exist", i, sni.key_path));
        }
    ...
    for route in &config.route {
        ...
        if !has_upstream {
            errors.push(format!("route '{}' references unknown upstream '{}'", route.path, route.upstream));
        }

        // WAF profile reference must exist
        if let Some(ref profile) = route.waf_profile {
            if !config.waf_profile.contains_key(profile) {
                errors.push(format!("route '{}' references unknown waf_profile '{}'", route.path, profile));
            }
        }
        ...
        if let Some(ref profile) = route.cache_profile {
            if !config.cache_profile.contains_key(profile) {
                errors.push(format!("route '{}' references unknown cache_profile '{}'", route.path, profile));
            }
        }
    }
    ...
    for (name, up) in &config.upstream {
        if up.url.parse::<hyper::Uri>().is_err() {
            errors.push(format!("upstream.{}.url '{}' is not a valid URL", name, up.url));
        }
    }
    ...
    if !errors.is_empty() {
        eprintln!("FATAL: config validation failed ({})", path);
        for e in &errors {
            eprintln!("  - {}", e);
        }
        std::process::exit(1);
...
/// Resolve upstream name to URL. Checks new [upstream.X] first, then legacy [upstreams].
fn resolve_upstream(config: &ZionConfig, name: &str) -> String {
    ...
    panic!("Unknown upstream '{}' — define it in [upstream.{}] or [upstreams]", name, name);
}
...
pub fn build_router(config: &ZionConfig) -> Router<Arc<ResolvedRoute>> {
    ...
    for route in &config.route {
        ...
        let waf = if let Some(ref profile_name) = route.waf_profile {
            Some(
                config
                    .waf_profile
                    .get(profile_name)
                    .unwrap_or_else(|| panic!("Unknown waf_profile '{}' in route {}", profile_name, route.path))
                    .clone(),
            )
        } else if route.waf {
        ...
        let cache = if let Some(ref profile_name) = route.cache_profile {
            Some(
                config
                    .cache_profile
                    .get(profile_name)
                    .unwrap_or_else(|| panic!("Unknown cache_profile '{}' in route {}", profile_name, route.path))
                    .clone(),
            )
        ...
        // Pre-parse upstream URI at startup — zero parse cost at runtime
        let upstream_uri: hyper::Uri = upstream_url.parse()
            .unwrap_or_else(|e| panic!("Invalid upstream URL '{}': {}", upstream_url, e));
        let upstream_scheme = upstream_uri.scheme().cloned()
            .unwrap_or_else(|| "http".parse().unwrap());
        let upstream_authority = upstream_uri.authority().cloned()
            .unwrap_or_else(|| panic!("Upstream '{}' has no authority", upstream_url));
        ...
        let auth = route.auth_profile.as_ref().map(|name| {
            let profile_config = config.auth_profile.get(name)
                .unwrap_or_else(|| panic!("Auth profile '{}' not found (route '{}')", name, route.path));
            let resolved = crate::auth::resolve_auth_profile(profile_config);
            eprintln!("  auth: route {} → profile '{}' (alg={})", route.path, name, profile_config.algorithm);
            resolved
        });
        ...
        eprintln!(
            ...
            route.path,
            route.upstream,
            route.waf_profile.as_deref().unwrap_or(if route.waf { "legacy" } else { "off" }),
            route.cache_profile.as_deref().unwrap_or(if route.mode == RouteMode::StaticCache { "auto" } else { "off" }),
            route.mode,
        );
    }
...
"#
    ...
    fn parse_named_upstream() {
        ...
        assert_eq!(api.url, "http://127.0.0.1:8000");
    ...
    fn build_router_with_legacy_upstreams() {
        ...
        assert_eq!(matched.value.upstream_url, "http://127.0.0.1:8000");
    ...
    fn build_router_with_named_profiles() {
        ...
        assert_eq!(api.value.upstream_url, "http://127.0.0.1:8000");
        ...
        assert_eq!(statics.value.upstream_url, "http://127.0.0.1:3000");
...
"#;
        ...
        assert_eq!(route.value.upstream_url, "http://new:9000");
    }
}
