...
pub fn resolve_auth_profile(config: &AuthProfileConfig) -> ResolvedAuthProfile {
    ...
    } else if let Some(ref _jwks_url) = config.jwks_url {
        // TODO: fetch JWKS from URL at startup (requires async HTTP client)
        // For now, panic with clear message
        panic!("JWKS URL-based auth requires manual key setup. Use 'secret' for HMAC or pre-fetch JWKS.");
    } else {
        panic!("Auth profile requires either 'secret' or 'jwks_url'");
    };
...
mod tests {
    ...
    fn extract_bearer_valid() {
        assert_eq!(extract_bearer("Bearer eyJhbGciOiJIUzI1NiJ9"), Some("eyJhbGciOiJIUzI1NiJ9"));
    }
    ...
    fn validate_wrong_secret_fails() {
        ...
        let claims = Claims {
            ...
            email: None, iss: None, aud: None,
            exp: Some(u64::MAX), nbf: Some(0),
        };
