...
pub fn detect() -> &'static Platform {
    PLATFORM.get_or_init(|| {
        ...
        let platform = Platform {
            ...
            has_so_reuseport: cfg!(any(target_os = "linux", target_os = "macos", target_os = "freebsd")),
...
pub fn print_report(p: &Platform) {
    ...
    eprintln!("  │ CPUs:  {:<10} RAM:  {} MB{:>14}│", p.cpu_cores, p.ram_mb, "");
    eprintln!("  ├────────────────────────────────────────────┤");
    eprintln!("  │ CPU Features:                              │");
    eprintln!("  │   AES-NI/CE:  {:<5}  SHA-256:  {:<12}│", yn(p.has_aes_ni), yn(p.has_sha256));
    eprintln!("  │   NEON/SIMD:  {:<5}  AVX2:     {:<12}│", yn(p.has_neon), yn(p.has_avx2));
    eprintln!("  ├────────────────────────────────────────────┤");
    eprintln!("  │ OS Features:                               │");
    eprintln!("  │   SO_REUSEPORT:  {:<5}  TCP_FASTOPEN: {:<5}│", yn(p.has_so_reuseport), yn(p.has_tcp_fastopen));
    eprintln!("  │   TCP_QUICKACK:  {:<28}│", yn(p.has_tcp_quickack));
    eprintln!("  ├────────────────────────────────────────────┤");
    eprintln!("  │ CPU Cache:                                 │");
    eprintln!("  │   Line: {}B  L1d: {}KB  L2: {}KB{:>10}│",
              p.cache_line_size, p.l1d_cache_size / 1024, p.l2_cache_size / 1024, "");
    eprintln!("  │   L1 hot entries: {:<25}│", p.l1_hot_entries);
    eprintln!("  ├────────────────────────────────────────────┤");
    eprintln!("  │ Auto-tuning:                               │");
    eprintln!("  │   Workers:     {:<6} Conn limit:  {:<8}│", p.worker_threads, p.conn_limit);
    eprintln!("  │   Backlog:     {:<6} Buf size:    {:<5}KB │", p.backlog, p.send_buf / 1024);
    eprintln!("  └────────────────────────────────────────────┘");
}
...
fn detect_cache_line_size() -> usize {
    #[cfg(target_os = "macos")]
    {
        sysctl_usize(b"hw.cachelinesize\0").unwrap_or(
            if cfg!(target_arch = "aarch64") { 128 } else { 64 }
        )
    }
    #[cfg(target_os = "linux")]
    {
